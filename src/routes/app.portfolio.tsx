...
export const Route = createFileRoute("/app/portfolio")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Open exposure, claimable payouts and refunds on Studio Next." },
...
function Portfolio() {
  const positions = useQuery({ queryKey: ["positions"], queryFn: () => primacy.listPositions() });
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <Metric label="Open exposure" value={`${gen(openExposure)} GEN`} sub="stakes in live hours" />
        <Metric label="Claimable" value={`${gen(claimable)} GEN`} sub="payouts and refunds waiting" />
      </div>
                  ...
                  <tr key={p.marketId} className="h-14 hover:bg-surface-2">
                    ...
                    <td className="px-6 text-right">
                      <Button
                        variant="outline"
                        className="h-9 px-4"
                        disabled={!canClaim || !HAS_CONTRACT}
                        reason={
                          !HAS_CONTRACT
                      ...
                      >
                        {p.state === "INCONCLUSIVE" ? "Refund" : "Claim"}
                      </Button>
                    </td>
                  </tr>
                );
